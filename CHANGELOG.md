...
- **Breaking:** `--no-gitignore`/`respect_gitignore: false` now also disables `ignore`'s hidden-file filter, so dot-files/dot-directories are walked too, not just `.gitignore`-excluded ones. Previously, disabling gitignore respect still silently skipped hidden entries; now `respect_gitignore: false` genuinely means "include everything" except `.fyaiignore`/`exclude_dirs`/`exclude_files` matches.
