...
fn main() -> io::Result<()> {
    ...
    let extensions: Option<Vec<String>> = matches.get_one::<String>("extensions")
        .map(|ext| ext.split(',').map(|s| s.trim().to_string()).collect());
    let min_size: u64 = matches.get_one::<String>("min_size")
        .and_then(|s| s.parse().ok())
        .unwrap_or(51_200); // 50KB default
    let max_size: Option<u64> = matches.get_one::<String>("max_size")
        .and_then(|s| s.parse().ok());
